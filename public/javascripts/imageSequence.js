...
function generateGamesquence(ispractice){
	...
	//generate non-repeating random number, the range is the number of all images
	var imageArr = getRandom(totalImageCount,31,fillerCount + targetCount);
	...
	for(var i = 0; i < levelCount; i++){
		...
		for(var j = 0; j < levelImageCount; j++){
			...
			if(imageInfoArr[i][j][0] > fillerCount){				
				imageInfoArr[i][j][2] = 1;
				//console.log(j,imageInfoArr[i][j]);
				...
				if(j <= levelImageCount - targetSpacingMin - 1 && imageInfoArr[i][j][1] != 1){
					...
					if(imageInfoArr[i][k][1] != 1){
						//console.log(k,imageInfoArr[i][k]);
						imageInfoArr[i][k][0] = imageInfoArr[i][j][0];  //repeat image
